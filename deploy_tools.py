"""
Tools to package and deploy the lambda function for the Brookline voice app.
"""
...
LAMBDA_REL_PATH = 'brooklinevoiceapp/lambda_function.py'
LAMBDA_FUNCTION_PATH = os.path.join(PROJECT_ROOT, LAMBDA_REL_PATH)
INTERACTION_MODEL_REL_PATH = 'brooklinevoiceapp/interaction_models/en_US.json'
...
def package_lambda_function():
    ...
    shutil.copytree(PROJECT_PATH, os.path.join(TEMP_DIR_PATH))
...
def update_interaction_model(provided_skill_id):
    """
    Upload the interaction model JSON
    ...
    if skill_id == 'Env_Var':
        if 'BROOKLINE_VOICE_APP_SKILL_ID' in os.environ:
            skill_id = os.environ['BROOKLINE_VOICE_APP_SKILL_ID']
        else:
            print('! Error: Unable to update interaction model.\n'
                  '! Please provide a skill ID with -i or define a\n'
                  '! BROOKLINE_VOICE_APP_SKILL_ID environment variable.')
            print(HORIZONTAL_RULE)
            return
